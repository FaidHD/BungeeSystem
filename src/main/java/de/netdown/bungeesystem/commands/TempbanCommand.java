...
public class TempbanCommand extends Command {
    ...
    public TempbanCommand(BungeeSystem plugin) {
        super("tempban");
        this.plugin = plugin;
        plugin.getProxy().getPluginManager().registerCommand(plugin, this);
    }

    // /tempban <Spieler> <Zeit> <Einheit> <Punkte> <Grund>
                    ...
                    try {
                        ...
                        if (time > 2592000 && time != 0)
                            if (!commandSender.hasPermission("bungee.ban.all")) {
                                commandSender.sendMessage(new TextComponent(plugin.getData().getPrefix() + "Du darfst nicht mehr als 30 Tage bannen."));
                                return;
                            }
                        int points = Integer.parseInt(args[3]);
                        if (points > 3)
                            if (!commandSender.hasPermission("bungee.ban.all")) {
                                commandSender.sendMessage(new TextComponent(plugin.getData().getPrefix() + "Du darfst nicht mehr als 3 Banpunkte vergeben."));
                                return;
                            }
                        ...
                        Reason reason = new Reason(args[4].toUpperCase(), time, 0, points, Reason.ReasonType.BAN, false);
                        ...
                        commandSender.sendMessage(new TextComponent(plugin.getData().getPrefix() + "Bitte benutze: /tempban <Spieler> <Zeit> <s, m, h, d, w> <Punkte> <Grund>"));
                    }
                } else
                    commandSender.sendMessage(new TextComponent(plugin.getData().getPrefix() + "Bitte benutze: /tempban <Spieler> <Zeit> <s, m, h, d, w> <Punkte> <Grund>"));
            } else
